...
zpmdbtn?.addEventListener('click', ()=>{
    while (addrlist.lastChild) {
        addrlist.removeChild(addrlist.lastChild);   //글씨 초기화
    }
    dong.value ='';
    modal.show();//모달 띄우기
});
...
const  showzipaddr = (jsons) => {
  ...
  //   }
    ...
    jsons.forEach(function (data, idx){
       let bunji = (data['bunji'] != null) ? data['bunji'] : ''; //null일때 제거
...
sendzip?.addEventListener('click', ()=>{
    ...
    if (addr !== ''){
        //  123-456 서울 구로구 구로1동
        let  zip = addr.split(' ')[0];  //공백으로 나눠 1번째거 (123-456)
